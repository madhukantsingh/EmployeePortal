...
const ApprovedRejectedDependent = () => {
  ...
  const { emp_id, lid,employee_name,dependent_name,relation,status} = user;
  ...
  const onSubmit = async e => {
    console.log(user)
    e.preventDefault();
    await axios.put(`http://127.0.0.1:8000/putdependents/${id}`, user)
    .then(res=>res.json())
    .then((result)=>{
        alert(result);
 ...
 navigate('/LeaveManagement/applyleave/approveleave');
  ...
  const loadUser = async () => {
    const result = await axios.get(`http://127.0.0.1:8000/individualDependent/${id}`);
    setUser(result.data);
  };
  return (
    <div>
...
export default ApprovedRejectedDependent;
